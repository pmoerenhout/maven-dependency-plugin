 ...
 * @since 2.2
 */
@Mojo( name = "analyze-repositories-2", requiresDependencyResolution = ResolutionScope.TEST, threadSafe = true )
public class AnalyzeRepositories2Mojo
    ...
    */
        ...
        throws MojoExecutionException
    {
        ...
        try
        {
            ...
            for ( Map.Entry<ArtifactRepository, List<Artifact>> entry
                : collectResult.getRemoteRepositoriesArtifacts().entrySet() )
            {
                ArtifactRepository artifactRepository = entry.getKey();
                Set<String> locations = new HashSet<String>();
                getLog().info( artifactRepository.getId() );
                getLog().info( entry.getValue().toString() );
                // Fetch mirrors from settings
                for ( Mirror mirror : settings.getMirrors() )
                {
                    if ( mirror.getId().equals( artifactRepository.getId() )
                        && mirror.getUrl().equals( artifactRepository.getUrl() ) )
                    {
                        locations.add( "Maven (user/global) settings" );
                    }
                }
                for ( Artifact artifact : entry.getValue() )
                {
                    traversePom( artifactRepository, artifact,
                        getMavenProject( ArtifactUtils.key( artifact ) ),
                        locations );
                }
                listRepository( artifactRepository , locations );
            }
        }
